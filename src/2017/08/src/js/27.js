...
function CircleEntity(x, y, radius, type, restitution, deceleration) {
  ...
  this.collidedWithCircle = function (peer) { // 円と円の衝突
    const d2 = Math.pow(peer.x - this.x, 2) + Math.pow(peer.y - this.y, 2);
...
function init() {
  const r = new RectangleEntity(550, 50, 50, 400, BodyStatic);
  r.color = "green";
  engine.entities.push(r);

  const rr = new RectangleEntity(0, 50, 50, 400, BodyStatic);
  rr.color = "yellow";
  engine.entities.push(rr);

  const l = new LineEntity(50, 300, 400, 350);
  l.color = "white";
  engine.entities.push(l);

  const ll = new LineEntity(500, 400, 100, 450);
  ll.color = "white";
  engine.entities.push(ll);

  for(let i=0; i<7; i++) {
    for(let j=0; j<3; j++) {
      const c = new CircleEntity(i*60+100, j*60+100, 5, BodyStatic);
      c.color = colors[j];
      engine.entities.push(c);
    }
  }

  for(let i=0; i<20; i++) {
    const c = new CircleEntity(rand(400)+50, rand(200), 10, BodyDynamic);
    c.color = colors[rand(5)];
    c.velocity.x = rand(10) - 5;
    c.velocity.y = rand(10) - 5;
    engine.entities.push(c);
  }
